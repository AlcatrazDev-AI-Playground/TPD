...
class ImageLogger(Callback):
    ...
    def _testtube(self, pl_module, images, batch_idx, split):
        for k in images:
            ...
            tag = f"{split}/{k}"
            if split == "train":
                pl_module.logger.experiment.add_image(tag, grid, global_step=batch_idx)
            else:
                pl_module.logger.experiment.add_image(
                    tag, grid, global_step=pl_module.global_step + batch_idx
                )
    ...
    def log_local(self, save_dir, split, images, global_step, current_epoch, batch_idx):
        ...
        for k in images:
            ...
            if self.rescale:
                if k == "conditioning":
                    grid[0, :, :] = grid[0, :, :] * 0.26862954 + 0.48145466
                    grid[1, :, :] = grid[1, :, :] * 0.26130258 + 0.4578275
                    grid[2, :, :] = grid[2, :, :] * 0.27577711 + 0.40821073
                else:
                    grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            self.check_frequency(check_idx, split)
            ...
            logger_log_images(pl_module, images, check_idx, split)
            if is_train:
                pl_module.train()
...
if __name__ == "__main__":
    ...
    # tensorboard logger
    logger = TensorBoardLogger(save_dir=logdir, name="my_model")
